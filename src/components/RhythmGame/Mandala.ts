...
export class Mandala extends Phaser.GameObjects.Container {
    ...
    constructor(scene:MusicScene, x:number, y:number, pt: Prop, tex:string, maxscale: number, rotation:number, expandtime: number, alpha: number=1, initsound: boolean = false, sound: string = "charge_big") {
        super(scene,x,y);
        this.owner = pt;
        this.scene=scene;
